<div class="padded form-agregar">
    ...
    <form class="form-horizontal" method="post" action="<?php echo site_url($this->uri->uri_string()); ?>">
        <dl>
            <input type="hidden" name="" value="<?php echo $this->mdl_empleados->form_value(''); ?>" />
        </dl>
        <div class="control-group <?php echo form_error('idempleado') != '' ? 'error' : '';?>">
            <label class="control-label">* idempleado </label>
            <div class="controls">
                <input type="text" name="idempleado" value="<?php echo $this->mdl_empleados->form_value('idempleado'); ?>" />
            </div>
        </div>
