...
class qqFileUploader {
    ...
    private function checkServerSettings(){
        ...
        if ($postSize < $this->sizeLimit || $uploadSize < $this->sizeLimit){
            ...
            die(json_encode(array('error'=>'incrementa post_max_size y upload_max_filesize a ' . $size)));
        }
    }
    ...
    function handleUpload($uploadDirectory, $replaceOldFile = FALSE) {
        ...
        if (!is_writable($uploadDirectory)){
            return array('error' => "Error del Servidor. el directorio de subida no es escribible.");
        }

        if (!$this->file){
            return array('error' => 'ningun archivo fue subido.');
        }
        ...
        if ($size > $this->sizeLimit) {
            return array('error' => 'El archivo es muy grande, el max es ' . $this->sizeLimit / 1024 / 1024 . "MB");
        }
        ...
        if($this->allowedExtensions && !in_array(strtolower($ext), $this->allowedExtensions)){
            ...
            return array('error' => 'El archivo tiene una extension invalida, deberia ser uno de los sgtes: '. $these . '.');
        }
        ...
        if ($this->file->save($uploadDirectory . DIRECTORY_SEPARATOR . $filename . $ext)){
            ...
            return array('error'=> 'No se pudo guardar la imagen subida.' .
                'La subida fue cancelada, o se a encontrado un error en el servidor');
        }
